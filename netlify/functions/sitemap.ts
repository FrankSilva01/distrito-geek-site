...
export default async () => {
  ...
  const paths = ['/', '/categoria/todos', ...SEO_LANDINGS.map((landing) => landing.path), ...categories.map((category) => `/categoria/${category}`), ...products.map((product) => `/produto/${product.slug}`), '/faq', '/contato', '/politica-de-privacidade', '/termos']
