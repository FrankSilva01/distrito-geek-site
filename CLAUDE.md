...
> - **RESOLVIDO em 30/08. O bug era do FlowOps, e foi corrigido lá.** `syncMlListings` pedia uma única página — `items/search?limit=50` — e ignorava `data.paging.total`. Quando a conta passou de 50 anúncios, tudo que sobrava ficou invisível para o sync, e anúncio novo nunca era descoberto. Junto vinha um segundo sintoma: três chamadas ao ML por anúncio, em série, e a Edge Function morria no meio — `HTTP 500` em 18.253 ms com `shutdown` no log, 17 registros atualizados em 29/08 e 28 em 23/08. A correção pagina de verdade e processa anúncio não registrado primeiro, depois o mais defasado, dentro de um orçamento de tempo. Repo `FrankSilva01/flowops-interno`, PR #1, já deployado.
> - **Segundo bug, desenterrado pelo primeiro:** `supabase.from("price_history").insert({...}).catch(...)` estourava `insert(...).catch is not a function` — o builder do PostgREST é *thenable* mas não expõe `.catch`. A linha só roda quando o preço de um anúncio muda, e com o `limit=50` o sync nem chegava nesses anúncios; corrigida a paginação, o caminho passou a ser exercitado e o erro apareceu na primeira corrida. Trocado por `try/catch` em volta do `await`. Depois disso: **47 dos 55 registros atualizados numa corrida**, contra 17 antes — os 8 restantes são registros que o ML não lista mais.
> - **Onde o FlowOps mora, para a próxima vez:** `C:\Users\Frank\Desktop\flowops-interno`, função em `supabase/functions/marketplace-sync/index.ts`, projeto Supabase `djvrhvzjvnyensbobtby`. O front deploya pelo Netlify a cada push; a **Edge Function não** — precisa de `npx supabase functions deploy marketplace-sync --project-ref djvrhvzjvnyensbobtby`, e o CLI exige login interativo num terminal de verdade.
> - Depois do deploy: `/storefront` foi de 53 para 55 registros e o Distrito Geek de 33 para 35 produtos, com `DG-MIN-000054` (Kit 9 Criaturas Bestiais) e `DG-MIN-000055` (Kit 5 Criaturas Demoníacas) gerados pelo próprio gerador de SKU. `MLB6940678098` continua fora porque o anúncio está indisponível no ML.
> - Cuidado que sobrou: a org Supabase está no plano **Free com a carência vencida**. Se a cota estourar o projeto para de responder, e aí cai o `/storefront` inteiro, não só o sync.
...
- O formatador de descrição remove Markdown simples recebido dos marketplaces (`###`, `**`, `__`, crase) e transforma headings/listas em blocos visuais; React continua escapando o conteúdo e nenhum HTML externo é injetado.
...
- Validação desta rodada: 333 testes, typecheck e build Vite aprovados. Bundle público: 135,40 → 135,99 kB gzip; Admin lazy: 132,04 → 134,92 kB gzip; GuidePage e Radar inalterados.
