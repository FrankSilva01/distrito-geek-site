...
> - **O FlowOps não descobre anúncio novo — só atualiza o que já está registrado nele.** Diagnóstico completo em 30/08: o painel do ML tem 48 anúncios, o FlowOps devolve 52 registros, e a interseção mostra **três anúncios do painel ausentes** (`MLB7546463124`, `MLB7492964436` e `MLB6940678098`) contra **sete registros mortos** que o FlowOps guarda para sempre (dois "Qa Codex" de teste, o orc finalizado, inativos e pausados). O endpoint está vivo: as corridas rodaram em 23/08 (28 registros, 21 s) e 29/08 (17 registros, 12 s), sempre mexendo em parte da lista — o que também explica registros parados desde junho. Ou seja: o job de refresh funciona, o passo de descoberta não existe ou nunca é alcançado. **A correção é registrar o anúncio no FlowOps**; a vitrine está fiel ao que o upstream entrega. Só `/storefront` está exposto no projeto Supabase (`sync`, `import`, `refresh` etc. dão 404), então não dá para disparar reimportação de fora.
> - Ponte enquanto isso: `scripts/importar-anuncios-ausentes.mjs`. `listCuratedProducts` junta sincronizado + interno e o **sincronizado ganha** no id repetido, então o que entra pelo script é descartado sozinho quando o FlowOps assumir o anúncio. O SKU DG continua vindo de `assignSkus`. Limite: é um retrato — preço, estoque e fotos não seguem o ML até o FlowOps assumir. `MLB6940678098` ficou de fora de propósito porque o anúncio está indisponível no ML e viraria página pública sem canal de compra.
