...
const LANDING_KIND_LABEL: Record<string, string> = { guide: "Guia", product: "Produto", category: "Categoria", other: "Outro", unknown: "Não identificada" };
        ...
        {activeSection === "analytics" && <div className="admin-card analytics-card">
          ...
          {analyticsError ? <div className="form-error" role="alert">{analyticsError}</div> : !analytics ? <p>Carregando dados de aquisição…</p> : !analytics.configured || !analytics.totals ? <p>Integrações analíticas ainda não configuradas{analytics.missing?.length ? `: ${analytics.missing.join(", ")}` : "."}</p> : <>
            ...
            <div className="stats analytics-stats">
              <div><span>Usuários</span><b>{analytics.totals.users}</b></div><div><span>Sessões</span><b>{analytics.totals.sessions}</b></div><div><span>Visualizações</span><b>{analytics.totals.pageViews}</b></div><div title="Todo pageview de /produto/* no GA4, inclusive de produto despublicado ou com slug antigo. O “Views de produtos publicados” do funil comercial conta só o catálogo publicado hoje, por isso é menor."><span>Visualizações de páginas de produto</span><b>{analytics.totals.productViews}</b></div><div><span>Cliques ML</span><b>{analytics.totals.mercadoLivreClicks}</b></div><div><span>Cliques Shopee</span><b>{analytics.totals.shopeeClicks}</b></div><div><span>Cliques TikTok</span><b>{analytics.totals.tiktokClicks || 0}</b></div><div><span>Cliques WhatsApp</span><b>{analytics.totals.whatsappClicks || 0}</b></div><div><span>CTR comercial</span><b>{(analytics.totals.ctr * 100).toFixed(1)}%</b></div>
            </div>
            ...
            {analytics.organicLandings?.length ? <div className="table-wrap"><table><thead><tr><th>Landing</th><th>Tipo</th><th>Usuários</th><th>Sessões</th><th>Cliques</th><th>Impressões</th><th>CTR</th><th>Posição</th></tr></thead><tbody>{analytics.organicLandings.map((row) => <tr key={row.path}><td>{row.kind === "unknown" ? <span title="O GA4 não conseguiu atribuir a landing desta sessão. As métricas são reais; a página de entrada é que é desconhecida.">Origem não identificada</span> : <a href={row.path}>{row.path}</a>}</td><td>{LANDING_KIND_LABEL[row.kind] || row.kind}</td><td>{row.users}</td><td>{row.sessions}</td><td>{row.clicks}</td><td>{row.impressions}</td><td>{(row.ctr * 100).toFixed(1)}%</td><td>{row.position ? row.position.toFixed(1) : "—"}</td></tr>)}</tbody></table></div> : <p>Sem entradas por busca orgânica no período.</p>}</section>
