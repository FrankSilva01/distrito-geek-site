...
export function AdminPage() {
  ...
  async function login(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setError(''); const form = new FormData(event.currentTarget); try { const response = await fetch('/api/admin-login', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ email: form.get('email'), password: form.get('password') }) }); const data = await response.json(); if (!response.ok) throw data; setAuth('yes'); await loadProducts() } catch (err) { setError(messageOf(err)) } }
  async function logout() { await fetch('/api/admin-session', { method: 'DELETE' }); setAuth('no') }
  async function readFile(file: File) { const book = XLSX.read(await file.arrayBuffer()); setPreview(XLSX.utils.sheet_to_json(book.Sheets[book.SheetNames[0]], { defval: '' })) }
  async function confirmImport() { setError(''); const imported = preview.map((row, index) => normalizeMarketplaceRow(row, index + 2)).filter((result) => !result.ignored && result.product && !result.errors.length).map((result) => result.product!); try { const response = await fetch('/api/admin-import', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ products: imported }) }); const data = await response.json(); if (!response.ok) throw data; setNotice(`${data.accepted} anúncios importados como rascunho.`); setPreview([]); await loadProducts() } catch (err) { setError(messageOf(err)) } }
  async function createProduct(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setError(''); const form = new FormData(event.currentTarget), title = String(form.get('title')), marketplace = String(form.get('marketplace')) as Marketplace, url = String(form.get('url')), image = String(form.get('image')), now = new Date().toISOString(); const product: Product = { id: `manual-${crypto.randomUUID()}`, slug: `${slugify(title)}-${Date.now()}`, title, description: String(form.get('description')), price: Number(form.get('price')), currency: 'BRL', stock: Number(form.get('stock') || 0), status: 'draft', category: 'colecionaveis', images: image ? [image] : [], attributes: {}, featured: false, showOnStorefront: true, listings: [{ marketplace, externalId: `manual-${Date.now()}`, url, active: true }], version: 1, createdAt: now, updatedAt: now }; try { const response = await fetch('/api/admin-products', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify(product) }); const data = await response.json(); if (!response.ok) throw data; setNotice('Anúncio salvo como rascunho.'); event.currentTarget.reset(); await loadProducts() } catch (err) { setError(messageOf(err)) } }
