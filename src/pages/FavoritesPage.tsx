...
export function FavoritesPage() {
  ...
  return <main className="container page collection-page"><p className="eyebrow">Sua seleção</p><h1>Favoritos</h1><p>Guarde produtos para rever depois. Preço e disponibilidade continuam vindo do catálogo sincronizado.</p>{products.length ? <div className="product-grid">{products.map((product) => product && <ProductCard key={product.id} product={product}/>)}</div> : <div className="empty collection-empty"><Heart/><h2>Nenhum favorito por enquanto</h2><p>Use o coração nos produtos para montar sua lista.</p><Link className="button primary" to="/categoria/todos">Explorar catálogo</Link></div>}</main>
}
